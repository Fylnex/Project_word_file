...
class StudentReportProcessor(DocumentProcessor):
    """
    Класс для обработки отчета по студентам.
    """
    ...
    def process(self):
        """Процесс обработки отчета по студентам."""
        try:
            self.load_template()
            self.replace_flags()
            self.insert_table()
    ...
    def create_table_with_borders(self):
        ...
        subheaders = ['№', 'ФИО', 'Группа', 'Зачетная книжка', '', '']
        ...
        for emloyee in self.employees_data:
            ...
            row[2].text = emloyee.get('Группа', '')
            row[3].text = emloyee.get('Зачетная книжка', '')
            row[4].text = ''
            row[5].text = ''
